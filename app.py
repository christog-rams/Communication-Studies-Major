...
SECTIONS = [
    ...
    },
    {
        ...
        "groups": [
            ...
            {
                "label": "Advanced writing — choose one (AUCC 2)",
                "choice": True,
                "courses": [
    ...
    },
    {
        ...
        "groups": [
            {
                "label": "SPCM electives (15 of 24 total required)",
                "courses": [
            ...
            {
                "label": "Minor or interdisciplinary minor (15 of 21 cr)",
                "minor": True,
                "courses": [
    ...
    },
    {
        ...
        "groups": [
            {
                "label": "Capstone (required)",
                "courses": [
                    {"code": "SPCM 479", "name": "Communication Studies Capstone", "aucc": "4C", "cr": 3},
                ],
            },
            {
                "label": "Upper-division SPCM — choose one (AUCC 4A/4B)",
                "choice": True,
                "courses": [
            ...
            {
                "label": "Remaining SPCM electives (9 cr)",
                "courses": [
                    {"code": "SPCM ***", "name": "Communication Studies elective", "aucc": "", "cr": 3},
                    {"code": "SPCM ***", "name": "Communication Studies elective", "aucc": "", "cr": 3},
                    {"code": "SPCM ***", "name": "Communication Studies elective", "aucc": "", "cr": 3},
...
def init_state():
    ...
    if "checked" not in st.session_state:
        st.session_state.checked = {}   # key → bool
    if "minor_names" not in st.session_state:
        st.session_state.minor_names = {}  # key → str
    if "extra_courses" not in st.session_state:
        st.session_state.extra_courses = {}  # sec_id → list of dicts

init_state()

# ── Helpers ────────────────────────────────────────────────────────────────────

def course_key(sec_id, gi, ci):
    return f"{sec_id}__{gi}__{ci}"

def is_checked(sec_id, gi, ci):
    return st.session_state.checked.get(course_key(sec_id, gi, ci), False)

def toggle(sec_id, gi, ci):
    k = course_key(sec_id, gi, ci)
...
def earned_credits():
    ...
    for sec in SECTIONS:
        for gi, g in enumerate(sec["groups"]):
            if g.get("choice"):
                # count only checked ones (should be at most one)
...
<style>
    ...
    .year-badge { display: inline-block; font-size: 11px; font-weight: 600; padding: 2px 10px;
                  border-radius: 99px; margin-right: 8px; }
    .aucc-pill { display: inline-block; font-size: 10px; background: #f0f0ec;
                 color: #888; border-radius: 99px; padding: 1px 7px; margin-left: 6px; }
...
col_title, col_name = st.columns([2, 1])
with col_title:
...
with col_name:
    st.session_state.student_name = st.text_input(
        ...
        placeholder="Enter student name", label_visibility="collapsed"
    )
...
with m1:
    st.markdown(f'<div class="metric-card"><div class="metric-label">Credits earned</div>'
                f'<div class="metric-value">{earned}</div>'
                f'<div class="metric-sub">of 120 required</div></div>', unsafe_allow_html=True)
with m2:
    st.markdown(f'<div class="metric-card"><div class="metric-label">Progress</div>'
                f'<div class="metric-value">{pct}%</div>'
                f'<div class="metric-sub">toward degree</div></div>', unsafe_allow_html=True)
with m3:
    st.markdown(f'<div class="metric-card"><div class="metric-label">SPCM credits</div>'
                f'<div class="metric-value">{spcm}</div>'
                f'<div class="metric-sub">of 24+ required</div></div>', unsafe_allow_html=True)
with m4:
    st.markdown(f'<div class="metric-card"><div class="metric-label">Remaining</div>'
                f'<div class="metric-value">{remaining}</div>'
                f'<div class="metric-sub">credits to complete</div></div>', unsafe_allow_html=True)
...
YEAR_BG     = {"Freshman": "#EEEDFE", "Sophomore": "#E1F5EE", "Junior": "#E6F1FB", "Senior": "#FAEEDA"}
...
for sec in SECTIONS:
    ...
    # Count completion for header
    all_courses = []
    for gi, g in enumerate(sec["groups"]):
        if g.get("choice"):
            all_courses.append(("choice", sec_id, gi, g["courses"]))
        else:
            for ci, c in enumerate(g["courses"]):
                all_courses.append(("regular", sec_id, gi, ci))

    done_count  = sum(1 for item in all_courses if item[0] == "regular" and is_checked(item[1], item[2], item[3]))
    total_count = sum(1 for item in all_courses if item[0] == "regular")
    done_count  += sum(1 for item in all_courses if item[0] == "choice" and any(is_checked(sec_id, item[2], ci) for ci in range(len(item[3]))))
    total_count += sum(1 for item in all_courses if item[0] == "choice")

    badge_text = "Complete" if done_count == total_count else ("In progress" if done_count > 0 else "Not started")

    with st.expander(f"**{year} year** — {done_count}/{total_count} complete · {badge_text}", expanded=(year == "Freshman")):
        ...
        for gi, group in enumerate(sec["groups"]):
            ...
            for ci, course in enumerate(group["courses"]):
                key = course_key(sec_id, gi, ci)
                col_chk, col_code, col_name, col_aucc, col_cr = st.columns([0.5, 1, 5, 1, 0.7])

                with col_chk:
                    checked = st.checkbox("", value=st.session_state.checked.get(key, False),
                                          key=f"chk_{key}", label_visibility="collapsed")
                    st.session_state.checked[key] = checked

                with col_code:
                    st.markdown(f"<span style='font-size:11px;font-family:monospace;color:#888'>{course['code']}</span>",
                                unsafe_allow_html=True)
                ...
                with col_name:
                    if is_minor:
                        ...
                        st.markdown(f"<span style='font-size:13px;{style}'>{course['name']}</span>",
                                    unsafe_allow_html=True)
                ...
                with col_cr:
                    st.markdown(f"<span style='font-size:12px;color:#aaa'>{course['cr']} cr</span>",
                                unsafe_allow_html=True)

            # Minor add-course button
            if is_minor:
                    ...
                    with mc4:
                        ...
                        if st.button("Add", key=f"madd_{sec_id}_{gi}"):
                            if m_name:
                                ...
                                extras.append({"code": m_code or "Minor", "name": m_name, "cr": m_cr, "done": False, "gi": gi})
                                st.rerun()

            st.markdown('<hr class="section-divider">', unsafe_allow_html=True)

        # Extra / added courses for this section
        ...
        if extras:
            ...
            for ei, extra in enumerate(extras):
                ...
                with ec1:
                    done = st.checkbox("", value=extra.get("done", False),
                                       key=f"extchk_{sec_id}_{ei}", label_visibility="collapsed")
                    extras[ei]["done"] = done
                with ec2:
                    st.markdown(f"<span style='font-size:11px;font-family:monospace;color:#888'>{extra['code']}</span>",
                                unsafe_allow_html=True)
                with ec3:
                    style = "text-decoration:line-through;color:#aaa;" if done else ""
                    st.markdown(f"<span style='font-size:13px;{style}'>{extra['name']}</span>",
                                unsafe_allow_html=True)
                with ec4:
        ...
        # Add any course to this section
        with st.expander("➕ Add a course to this year", expanded=False):
            ...
            with a4:
                ...
                if st.button("Add", key=f"aadd_{sec_id}"):
                    if a_name:
                        extras = st.session_state.extra_courses.setdefault(sec_id, [])
                        extras.append({"code": a_code or "—", "name": a_name, "cr": a_cr, "done": False})
                        st.rerun()
